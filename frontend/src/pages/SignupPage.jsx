...
export default function SignupPage() {
  ...
  const strong   = pwChecks.every(c => c.ok);

  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!strong) { setError('Please meet all password requirements.'); return; }
    setError('');
    setIsLoading(true);
    try {
  ...
  return (
    <div className="min-h-screen bg-paper flex items-center justify-center px-6 py-12">
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
        ...
        <div className="kicker mb-2">New account</div>
          ...
          <div>
            ...
            <div className="relative">
              <input type={showPass ? 'text' : 'password'} value={form.password}
                ...
                className="input-field w-full pr-10" placeholder="Min 8 characters" required />
            ...
            {form.password && (
              <div className="mt-2 grid grid-cols-2 gap-1">
                {pwChecks.map(c => (
                  <div key={c.label} className={`flex items-center gap-1.5 text-xs font-600 ${c.ok ? 'text-green-600' : 'text-muted'}`}>
                    <CheckCircle2 className={`w-3 h-3 flex-shrink-0 ${c.ok ? 'text-green-500' : 'text-rule'}`} />
                    {c.label}
                  </div>
                ))}
              </div>
        ...
        <p className="text-center text-sm text-muted mt-6">
          ...
          <Link to="/login" className="text-ink font-700 underline underline-offset-2 hover:text-accent">
            Sign in
          </Link>
        </p>
      </motion.div>
